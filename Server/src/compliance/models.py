...
class ComplianceRecordBase(BaseModel):
    requirement: str = Field(
        ..., description="Compliance requirement, e.g. GDPR Data Processing"
    )
    ...
    status: str = Field(
        ...,
        description="Compliance status: Compliant, Non-Compliant, Warning, Under Review",
    )
...
class DashboardSummaryResponse(BaseModel):
    compliance_score: float = Field(
        ..., description="Overall compliance score percentage, e.g. 84.0"
    )
    trend_value: float = Field(
        2.4, description="Overall compliance trend percentage, e.g. 2.4"
    )

    compliant_contracts: int = Field(..., description="Count of compliant contracts")
    compliant_contracts_trend: str = Field(
        "+12 this month", description="Compliant contracts card subtext"
    )

    critical_violations: int = Field(..., description="Count of critical violations")
    critical_violations_trend: str = Field(
        "Needs immediate action", description="Critical violations card subtext"
    )

    pending_audits: int = Field(..., description="Count of pending audits")
    pending_audits_trend: str = Field(
        "Scheduled for Q4", description="Pending audits card subtext"
    )
...
class PaginatedComplianceResponse(BaseModel):
    ...
    records: List[ComplianceRecordResponse] = Field(

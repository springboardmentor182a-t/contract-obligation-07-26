...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const linkClass =
    "flex items-center px-6 py-3.5 mb-1 font-medium transition-all duration-200 group";
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-40 w-72 bg-gradient-to-b from-[#0f172a] to-[#1e293b]
          border-r border-gray-800 text-white flex flex-col h-full overflow-y-auto
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-6 border-b border-gray-800/60 mb-4 sticky top-0 bg-[#0f172a]/90 backdrop-blur-md z-10">
          ...
          >
            ...
            <h2 className="m-0 text-2xl font-black tracking-tight text-white bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">
              ContractIQ
            </h2>
          </Link>
        ...
        <nav className="flex-1 pb-4">
          ...
          <Link
            to="/contracts"
            onClick={() => setIsOpen(false)}
            className={`${linkClass} ${isActive("/contracts")}`}
          >
          ...
          <Link
            to="/obligations"
            onClick={() => setIsOpen(false)}
            className={`${linkClass} ${isActive("/obligations")}`}
          >
          ...
          <Link
            to="/renewals"
            onClick={() => setIsOpen(false)}
            className={`${linkClass} ${isActive("/renewals")}`}
          >
          ...
          <Link
            to="/compliance"
            onClick={() => setIsOpen(false)}
            className={`${linkClass} ${isActive("/compliance")}`}
          >
          ...
          <Link
            to="/ai-analysis"
            onClick={() => setIsOpen(false)}
            className={`${linkClass} ${isActive("/ai-analysis")}`}
          >
        ...
        <nav className="pb-6">
          ...
          <Link
            to="/tax-estimators"
            onClick={() => setIsOpen(false)}
            className={`${linkClass} ${isActive("/tax-estimators")}`}
          >
            <div className="p-2 rounded-lg bg-indigo-500/10 text-indigo-400 group-hover:bg-indigo-500/20 group-hover:scale-110 transition-all mr-4">
